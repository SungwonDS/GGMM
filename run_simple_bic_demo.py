...
def run_demo(output_dir, run_sensitivity=True) -> DemoResult:
    out = Path(output_dir);out.mkdir(parents=True,exist_ok=True)
    truth = Mixture(**{name: np.asarray(values, dtype=float)
                       for name, values in TRUTH_PARAMETERS.items()})
    ...
    return DemoResult(ggmm=ggmm,truth=truth,x=x,comparison=comparison,selection=selection,
